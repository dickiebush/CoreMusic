...
def try_to_text(row):
    ...
    if (any([artist in row.artists for artist in my_artists])):
        body = "{} dropped a new song called {}, heres the link {}".format(row.artists, row.song_names, row.links)
...
## next steps include getting songs from other websites
## set up google form to let people use 
## host on django app to run faster 
